...
func GetSensorStatDesc(readingType SDRSensorReadingType, sensorType SDRSensorType, state2 uint8, state3 uint8) (string, [16]string) {
	sensorStatDescStr := ""
	var sensorEven [16]string
	if readingType == SENSOR_READTYPE_THREADHOLD {
		...
		for ; i < 8; i++ {
			if (state2 & (1 << i)) != 0 {
				sensorEven[index] = (discreteSensorStatDesc[uint8(readingType)])[i]
				index++
			}
		}
		for i = 0; i < 8; i++ {
			if (state3 & (1 << i)) != 0 {
				sensorEven[index] = discreteSensorStatDesc[uint8(sensorType)][i+8]
				index++
			}
		}
		...
		for ; i < 8; i++ {
			if (state2 & (1 << i)) != 0 {
				sensorEven[index] = sensorTypeCodeEvent[uint8(sensorType)][i]
				index++
			}
		}
		...
		for ; i < 8; i++ {
			if (state3 & (1 << i)) != 0 {
				sensorEven[index] = sensorTypeCodeEvent[uint8(sensorType)][i+8]
				index++
			}
		}
	}
...
func (c *Client) CalSdrRecordValue(recordType uint8, recordKeyBody_Data *bytes.Buffer) (*sDRRecordAndValue, error) {
	...
	if recordType == SDR_RECORD_TYPE_FULL_SENSOR {
		...
		if err != nil || (sensorReadingRes.ReadingAvail&0x20) == 0 {
			...
			sdrRecordAndValue.sensorStatDesc=""
			sdrRecordAndValue.sensorEvent=[16]string{""}
		} else {
